...
\textbf{Phương pháp đối sánh.} Chúng tôi sử dụng thuật toán \codeword{FedAvg} làm mô hình baseline. Để so sánh công bằng, chúng tôi cho phép mô hình này fine-tune một hoặc một vài bước trên một phần tập dữ liệu kiểm thử (thuật toán \codeword{FedAvgMeta}) trước khi bước vào kiểm thử thực sự. Các kết quả của thuật toán đề xuất sẽ được đem so sánh với kết quả của \codeword{FedAvg} và \codeword{FedAvgMeta}.
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|l|l|} 
    \hline
    \begin{tabular}[c]{@{}c@{}}\\Giai đoạn\end{tabular}                                       & \multicolumn{1}{c|}{Công việc}                                                               & \multicolumn{1}{c|}{Người thực hiện}  \\ 
    \hline
