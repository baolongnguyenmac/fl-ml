...
Các thuật toán \codeword{FedPer} và \codeword{FedPerMeta} được cài đặt và sử dụng trong huấn luyện mô hình để chứng minh việc các lớp phần chung của hệ thống chưa đạt hiệu quả cao khi làm việc trên tập dữ liệu của người dùng mới. Kết quả này cũng được đem so sánh với kết của của \codeword{FedMeta-Per}.
