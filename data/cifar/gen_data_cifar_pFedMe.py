...
cifar_data_image.extend(trainset.data.cpu().detach().numpy())
cifar_data_image.extend(testset.data.cpu().detach().numpy())
cifar_data_label.extend(trainset.targets.cpu().detach().numpy())
cifar_data_label.extend(testset.targets.cpu().detach().numpy())
cifar_data_image = np.array(cifar_data_image)
cifar_data_label = np.array(cifar_data_label)

cifar_data = []
for i in trange(10):
    idx = cifar_data_label==i
    cifar_data.append(cifar_data_image[idx])


print("\nNumb samples of each label:\n", [len(v) for v in cifar_data])
users_lables = []
...
for user in range(NUM_USERS):
    for j in range(NUM_LABELS):  # 3 labels for each users
        ...
        l = (user + j) % 10
        print("L:", l)
        X[user] += cifar_data[l][idx[l]:idx[l]+10].tolist()
...
props = np.array([[[len(v)-NUM_USERS]] for v in cifar_data]) * \
...
for user in trange(NUM_USERS):
    for j in range(NUM_LABELS):  # 4 labels for each users
        ...
        if idx[l] + num_samples < len(cifar_data[l]):
            X[user] += cifar_data[l][idx[l]:idx[l]+num_samples].tolist()
